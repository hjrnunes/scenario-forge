...
from scenario_forge.pipeline.generate import generate_scenario, write_scenario_outputs
...
def run_pipeline(
    ...
    profile_path.write_text(
        yaml.dump(profile_data, default_flow_style=False, sort_keys=False, allow_unicode=True),
        encoding="utf-8",
    )
    ...
    ts_path.write_text(
        yaml.dump(ts_data, default_flow_style=False, sort_keys=False, allow_unicode=True),
        encoding="utf-8",
    )
    ...
    for i, seed in enumerate(seeds, 1):
        ...
        try:
            envelope = generate_scenario(seed, profile, client, use_case)
