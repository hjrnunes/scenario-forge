...
class TestAtlasDerivedPatternsLoad:
    ...
    def test_pattern_has_required_fields(self, all_patterns: dict[str, dict], pid: str):
...
class TestAtlasDerivedPatternsValidate:
    ...
    def test_pydantic_validation(self, all_patterns: dict[str, dict], pid: str):
        """Pattern passes the explicit legacy catalogue boundary."""
        validated = validate_legacy_attack_pattern(all_patterns[pid])
        assert isinstance(validated, LegacyAttackPatternRecord)
    ...
    def test_kill_chain_present_and_valid(
        ...
        assert (
            len(validated.kill_chain) >= ATLAS_PATTERNS[pid]["min_kill_chain_steps"]
        ), (
    ...
    @pytest.mark.parametrize("pid", list(ATLAS_PATTERNS.keys()))
    def test_evidence_type_is_valid(self, all_patterns: dict[str, dict], pid: str):
